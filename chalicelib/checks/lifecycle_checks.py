...
def patch_file_lifecycle_status(connection, **kwargs):
    ...
    for file in files:
        ...
        try:
            ...
            if s3_tag:
                my_s3_util.set_object_tags(key=upload_key, bucket=file_bucket, tags=s3_tag, merge_existing_tags=True)
                ...
                if not is_extra_file:
                    ...
                    if(file_status == lifecycle_utils.ARCHIVED or file_status == lifecycle_utils.DELETED):
...
def check_deleted_files_lifecycle_status(connection, **kwargs):
    ...
    my_auth = connection.ff_keys
    check.action = "patch_deleted_files_lifecycle_status"
    check.description = (
...
@action_function()
def patch_deleted_files_lifecycle_status(connection, **kwargs):
    # start = datetime.utcnow()
    action = ActionResult(connection, 'patch_deleted_files_lifecycle_status')
    my_auth = connection.ff_keys
    env = connection.ff_env
    my_s3_util = s3Utils(env=env)
    raw_bucket = my_s3_util.raw_file_bucket
    ...
    for file in files:
        ...
        try:
            s3_tag = lifecycle_utils.lifecycle_status_to_s3_tag(lifecycle_utils.DELETED)
            if s3_tag:
                my_s3_util.set_object_tags(key=upload_key, bucket=file_bucket, tags=s3_tag, merge_existing_tags=True)
                ...
                if not is_extra_file:
                    today = datetime.date.today().strftime("%Y-%m-%d")
                    patch_dict = {
                        's3_lifecycle_status': lifecycle_utils.DELETED,
                        's3_lifecycle_last_checked': today
                    }                    
                    ff_utils.patch_metadata(patch_dict, uuid, key=my_auth)
            else: 
            ...
            log_message = f'File {uuid} ({upload_key}) has been tagged for deletion'
            action_logs['logs'].append(log_message)
            action_logs['patched_files'].append(uuid)
